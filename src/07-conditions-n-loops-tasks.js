...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${start}${firstChar}, ${secChar}${end}`;
}
...
function reverseInteger(num) {
  const resStrNum = String(num)
    .split('')
    .reverse()
    .join('');
  return Number(resStrNum);
}
...
function isCreditCardNumber(ccn) {
  ...
  let startInd = 0;
  if ((numArr.length - 1) % 2 === 0) startInd = 1;
  for (let i = startInd; i < numArr.length; i += 2) {
    let item = numArr[i] * 2;
    if (item > 9) item -= 9;
    numArr[i] = item;
  }
...
function getDigitalRoot(num) {
  ...
  if (resSum > 9) resSum = getDigitalRoot(resSum);
  return resSum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (elements[str[i]]) {
      if (resArr[resArr.length - 1] !== elements[str[i]]) {
        return false;
      }
      resArr.pop();
    } else {
